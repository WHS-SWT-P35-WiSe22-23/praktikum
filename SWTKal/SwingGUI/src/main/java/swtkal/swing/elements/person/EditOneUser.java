...
public class EditOneUser extends ListenerForActions
{
	...
	public EditOneUser()
	{
		gui = new JInternalFrame("Pers�nliche Daten", true, true, false, true);
	...
	void create()
	{
		...
		JLabel re = null;
		try {
			re = new JLabel(new ImageIcon(ImageIO.read(getClass().getClassLoader().getResourceAsStream("images/secure1.gif"))));
		} catch (IOException e) {
...
// altes K�rzel bleibt im Person-Objekt!			
...
//		}
		...
		// Passw�rter unterschiedlich
		if (!passwort[0].getText().equals(passwort[1].getText()))
		{
			String text = "Die Passw�rter sind nicht ident. Bitte tragen Sie das Passwort nochmals in beide Textfelder ein!\n";
			passwort[0].setText("");
			passwort[1].setText("");
