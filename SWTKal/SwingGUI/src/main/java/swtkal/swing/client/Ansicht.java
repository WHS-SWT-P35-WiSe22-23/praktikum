...
public class Ansicht implements ActionListener, MouseListener, ListDataListener, Observer //, EventConstants, Shared
{
...
//	}
    ...
    JPanel createSelectDate(String dateText1, String dateText2, boolean isfeiertag, Feiertag feiertag) throws IOException {
        JPanel selectDate = new JPanel();
        selectDate.setLayout(new BoxLayout(selectDate, BoxLayout.X_AXIS));

        JButton b1 = new JButton(new ImageIcon(ImageIO.read(getClass().getClassLoader().getResourceAsStream("images/left.gif"))));
        b1.setMargin(new Insets(0, 0, 0, 0));
        b1.setAlignmentY((float) 0.5);
        b1.setRolloverEnabled(true);
        b1.setRolloverIcon(new ImageIcon(ImageIO.read(getClass().getClassLoader().getResourceAsStream("images/leftRollover.gif"))));
        b1.setActionCommand("leftArrow");
        b1.addActionListener(this);

        JButton b2 = new JButton(new ImageIcon(ImageIO.read(getClass().getClassLoader().getResourceAsStream("images/right.gif"))));
        b2.setMargin(new Insets(0, 0, 0, 0));
        b2.setAlignmentY((float) 0.5);
        b2.setRolloverEnabled(true);
        b2.setRolloverIcon(new ImageIcon(ImageIO.read(getClass().getClassLoader().getResourceAsStream("images/rightRollover.gif"))));
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() instanceof TerminObjekt) {
            ...
            if (c.indexOf("eintragen") >= 0) {
                ...
                EditTerminControl editTermin = new EditTerminControl(parentFrame, client, termin.getBeginn());
...
//				{
                EditTerminControl editTermin = new EditTerminControl(parentFrame, client, termin);
                JInternalFrame gui = editTermin.getGUI();
...
//				}
            } else if (c.indexOf("l�schen") >= 0) {
                // geklickten Termin l�schen
//				if (termin.getOwner().getID() == Data.user.getID())
//				{
                try {
                    client.getServer().delete(termin);
                    // Aktualisieren der Tagesansicht ansto�en
                    client.actionPerformed(new ActionEvent(this, 0, "Tagesansicht"));
                } catch (TerminException te) {
                    showDialog("Termin l�schen", te.getMessage());
                }
//				}
//				else
//				{
//					showDialog("Termin l�schen",
//							"Sie sind nicht berechtigt, diesen Termin zu l�schen!");
//				}
            }
...
//								"Bitte tragen Sie in den UserProperties den vollst�ndigen Pfad fuer Ihren Browser ein!");
//					}
//
//				}
//				// if WINDOWS
...
//				else
//				{
//					showDialog("ToDo l�schen",
//							"Sie sind nicht berechtigt, diesen ToDo-Eintrag zu l�schen!");
//				}
//			}
//			else if (c.indexOf("Nachricht schicken") >= 0)
//			{
...
//			}
            ...
            if (c.indexOf("Termin") >= 0) {
                // neuen Termin eintragen
                EditTerminControl editTermin = new EditTerminControl(parentFrame, client, date);
                JInternalFrame gui = editTermin.getGUI();
...
//					&& toDo.getF�lligPer().isGreater(bgnAnsicht) >= 0
